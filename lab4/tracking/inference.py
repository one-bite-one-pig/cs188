...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            curnottojoin,joint_factor=joinFactorsByVariable(Factors,var)
            if len(joint_factor.unconditionedVariables())==1:
                Factors=curnottojoin
            else:
                eliminated_factor=eliminate(joint_factor,var)
                Factors=curnottojoin+[eliminated_factor]
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        for x,y in self.allPositions:
            result=self.getObservationProb(observation,gameState.getPacmanPosition(),(x,y),self.getJailPosition())
            self.beliefs[(x,y)]*=result
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            copy[pos]=self.beliefs[pos]
            self.beliefs[pos]=0
            
        for oldpos in self.allPositions:
            distribution=self.getPositionDistribution(gameState,oldpos)
            for newpos in self.allPositions:
                self.beliefs[newpos]+=copy[oldpos]*distribution[newpos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        all_loc= self.legalPositions
        num_of_loc=len(all_loc)
        
        aver=num//num_of_loc
        left=num%num_of_loc
        
        self.particles+=aver*all_loc
        
        for i in range(left):
            self.particles.append(all_loc[i])
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            belief[particle]+=self.getObservationProb(observation,pac_pos,particle,jail_pos)
